...
class Zend_View_Helper_Employeedetails extends Zend_View_Helper_Abstract {
	...
	public  function employeedetails($emparr,$conText,$userId)
	{
		...
		$emp_id = $emparr['id'];
		$projectsModal = new Default_Model_Projects;

		$empprojectname = $projectsModal->getProjectByEmpId($emp_id);

		$emp_project_details = "";
		
		foreach ($empprojectname as $value) {

			if($emp_project_details == ""){
				$emp_project_details .= $value["project_name"] ." (Start Date: ".$value["start_date"]."- End Date: ".$value["end_date"].")";	
			}
      ...
      <?php
		...
		**/
		...
		if(!empty($emp_project_details)){
			$empdata .=	'<p><b>Assigned Project <i>:</i></b><span>'.$emp_project_details.'</span></p>';
		}
		
		else{
			$empdata .=	'<p><b>Assigned Project <i>:</i></b><span>Project Not Assigned</span></p>';
		}
	...
	public  function employeetabs($conText,$userId)
	{
		...
		if($conText == "edit")
		{
			...
			// $tabsHtml .= '<li id="projectdetails" onclick="changeviewscreen(\'employee\','.$userId .');">
				// '.TAB_EMP_PROJECT.'</li>';

			// if(!empty($empOrganizationTabs) && in_array("emp_projects", $empOrganizationTabs))
			// $tabsHtml .= '<li id = "empprojects" onclick="changeviewscreen(\'empprojects\','.$userId .');">'.TAB_EMP_PROJECT.'<span class="beta_menu"></span></li>';
		...
		else if($conText == "view")
		{
			...
			// 	if(!empty($empOrganizationTabs) && in_array("projectdetails", $empOrganizationTabs))
			// $tabsHtml .= '<li id = "empprojects" onclick="changemyempviewscreen(\'empprojects\','.$userId .');">'.TAB_EMP_PROJECT.'<span class="beta_menu"></span></li>';
