...
class Default_Model_Leaverequest extends Zend_Db_Table_Abstract
{
	...
	public function getWFHLeaves($loginUserId,$year,$month)
	{
		$leavestatus= 'Cancel';
	 	$select = $this->select()
    					   ->setIntegrityCheck(false)	
                           ->from(array('e'=>'main_leaverequest'),array('id'=>'e.id','leavedays'=>'e.leaveday'))
                           ->where('e.user_id = ? ',$loginUserId)
                           ->where('e.leavetypeid = 6')
                           ->where('e.leavestatus <> ? ','Cancel')
                           ->where('year(e.from_date) = ? ',$year)
                           ->where('month(e.from_date) = ? ',$month);

						   // ->where('e.user_id='.$loginUserId.' and e.leavetypeid=6 AND leavestatus <> '.$leavestatus.' AND year(from_date) = '.$year.' and month(from_date) = '.$month.' AND e.isactive = 1');  		   					   				
		return $this->fetchAll($select)->toArray(); 
      }
