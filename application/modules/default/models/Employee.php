...
class Default_Model_Employee extends Zend_Db_Table_Abstract
{
	...
	*/
	...
	public function getEmployees($managerid='',$loginUserId,$limit,$offset,$search_str)
    {
    	$auth = Zend_Auth::getInstance();
    	$request = Zend_Controller_Front::getInstance();
     	if($auth->hasIdentity()){
		...
		if($search_str !='')
		{
			...
			// {
				...
				$where .= " AND e.userfullname like '".$search_str_r."%' or  e.employeeId like '".$search_str_r."%' or e.emprole_name like '".$search_str_r."%' or e.department_name like '".$search_str_r."%'";
			// }
       ...
       $employeesData = $this->select()
                                ...
                                // ->joinLeft(array('f'=>'tm_project_employees'), "f.emp_id = e.user_id and f.is_active = 1")
                                // ->joinLeft(array('g'=>'tm_projects'), "g.id = f.project_id and g.is_active = 1",array('projectname'=>'g.project_name'))                                  
                                ->where($where)
                                ->limit($limit, $offset)
