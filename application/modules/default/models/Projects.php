...
/* This is used in Advances for getting projects based on employee*/
		...
		$sql="SELECT p.project_name,p.start_date,p.end_date FROM tm_project_employees pe
		INNER JOIN tm_projects p ON p.id = pe.project_id
					WHERE emp_id=$to_id"; 		
	...
	public function getProjectsData($sort, $by, $pageNo, $perPage,$searchQuery)
	{
		...
		->from(array('p' => $this->_name),array('id'=>'p.id','project_name'=>'p.project_name','sow_number'=>'p.sow_number','sow_amount'=>'p.sow_amount','po_number'=>'p.po_number','po_amount'=>'p.po_amount','po_balance'=>'p.po_balance','project_status'=>'if(p.project_status = "initiated", "Initiated",if(p.project_status = "draft" , "Draft",if (p.project_status = "in-progress","In Progress",if(p.project_status = "hold","Hold",if(p.project_status = "completed","Completed","")))))','start_date'=>'p.start_date','end_date'=>'p.end_date','parent_project'=>'p2.project_name','project_type'=>'IF(p.project_type="billable","Billable",IF(p.project_type="non_billable","Non billable","Revenue generation"))'))
...
public function getclientname($unitid)
	...
	public function getEmpProjectsData($sort, $by, $pageNo, $perPage,$searchQuery,$employee_id)
	{
		...
		->joinLeft(array('p' => $this->_name),'tpe.project_id=p.id',array('id'=>'p.id','project_name'=>'p.project_name','sow_number'=>'p.sow_number','sow_amount'=>'p.sow_amount','po_number'=>'p.po_number','po_amount'=>'p.po_amount','po_balance'=>'p.po_balance','project_status'=>'p.project_status','start_date'=>'p.start_date','end_date'=>'p.end_date','base_project'=>'p.base_project','project_type'=>'IF(p.project_type="billable","Billable",IF(p.project_type="non_billable","Non billable","Revenue generation"))'))
		->joinLeft(array('c'=>'tm_clients'),'p.client_id=c.id',array('client_name'=>'c.client_name'))
		->joinLeft(array('cur'=>'main_currency'),'p.currency_id = cur.id',array('currencyname'=>'cur.currencyname'))
		->where($where)
		->order("$by $sort")
