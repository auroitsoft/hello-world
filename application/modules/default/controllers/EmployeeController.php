...
class Default_EmployeeController extends Zend_Controller_Action
{
	...
	public function autoempsearchAction(){

		$usersModel = new Default_Model_Users();
		$userDetails = $usersModel->autoEmpSearch($searchText);

		var_dump($userDetails);
		$searchName = $_POST['term'];
		var_dump($searchName);
	}
	...
	public function getmoreemployeesAction()
	{
			...
			if(	$empflag == 'myemployees')
			{
				...
				$dataTmp = $employeeModel->getEmployees('',$loginUserId,$limit,$offset,$search_str); 
             	$totalemployees= $employeeModel->getEmployees('',$loginUserId,'','',$search_str); 
				$totalcount=count($totalemployees);
				$empcount=count($dataTmp);
			}
				if($flag == 0)
	...
	public function viewAction()
	{
		...
		try
		{
			if($id && is_numeric($id) && $id>0 && $id!=$loginUserId)
			{
				...
				$data = array();
				$data = $projectModel->getProjectByEmpId($id);
				$data = $employeeModal->getsingleEmployeeData($id);
