...
class Default_IndexController extends Zend_Controller_Action
{
    ...
    try{
		...
		$client_id = "378258426408-50jmfd2t2u3e6ovg3ami656pf1lnnand.apps.googleusercontent.com";

		$id_token = $this->getRequest()->getParam ('id_token');
...
//		  $userid = $payload['sub'];
		  $domain = $payload['hd'];
//		  $this->_helper->getHelper("FlashMessenger")->addMessage( json_encode($email));
//          $this->_redirect('index');
//          return;

//          if($domain=="decisionminds.com"){
              $email = $payload['email'];
  //        }
		...
		// get username from db by using email
        $usersModel = new Default_Model_Users();
        $userDetails = $usersModel->getUserDetailsByEmail($email);
//		$options['username'] = $userDetails['emailaddress'] ;
        foreach($userDetails as $value)
        {
            $options['username'] = $value['emailaddress'];
        }
//        $this->_helper->getHelper("FlashMessenger")->addMessage( json_encode($options['username']));
//          $this->_redirect('index');
//          return;
		// $options['user_password']= $this->getRequest()->getParam('password');
		// $options['user_email']= $userid;;
		...
		try
		{
            ...
            $result = new Zend_Auth_Result($result['code'], $result['identity'], $result['messages']);
		...
		{
            $this->_helper->getHelper("FlashMessenger")->addMessage("hello". $e->getMessage());
				$this->_redirect('index');
			//echo $e->getMessage();
		}}
   catch(Exception $e)
