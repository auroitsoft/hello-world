	...
	public function saveleaverequestdetailsAction()
	{
	  ...
	  $auth = Zend_Auth::getInstance();
		...
		*/
		...
		{
			// echo $from_date;
			$from_datearr = explode('-',$from_date);
					$year_wfh = $from_datearr[0];//$from_date->format('m');
					$month_wfh = $from_datearr[1];//$from_date->format('Y');

						$getWFHLeavesarr = $leaverequestmodel->getWFHLeaves($loginUserId,$year_wfh,$month_wfh);
						
						if(!empty($getWFHLeavesarr))
								{
									for($i=0;$i<sizeof($getWFHLeavesarr);$i++)
									{
										$totalwfh = $totalwfh + $getWFHLeavesarr[$i]['leavedays'];
										$totalwfh1 = $totalwfh1 .',' . $getWFHLeavesarr[$i]['id'];
									}

									if ($totalwfh>=WHFDAYS)
									{
										$errorflag = 'false';
										$msgarray['from_date'] = 'You can have only 2 work from home leave in a month.';
									}
								}
		}
