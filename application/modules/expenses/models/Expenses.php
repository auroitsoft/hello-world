...
class Expenses_Model_Expenses extends Zend_Db_Table_Abstract
{
	...
	public function getExpensesData($sort, $by, $pageNo, $perPage,$searchQuery)
	{
		...
		if($loginUserId==1)
		{
			$where = "e.isactive = 1 and (e.status = 'approved' OR e.status = 'submitted')";
		}
		else if($loginUserId==257)
		{
			$where = "e.isactive = 1 and (e.status = 'approved')";
		}
		else
		{
	...
	{	 
		$db = Zend_Db_Table::getDefaultAdapter();
		$query = "select a.id,a.project_name,b.id,b.client_name as client from tm_projects a join tm_clients b on a.client_id = b.id where a.id =$id";
		$result = $db->query($query)->fetch();
		return $result;
