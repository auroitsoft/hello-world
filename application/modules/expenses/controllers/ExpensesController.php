...
class Expenses_ExpensesController extends Zend_Controller_Action
{
	...
	public function preDispatch()
	{
		...
		$ajaxContext->addActionContext('getclient', 'json')->initContext();
	}
	...
	public function expensestatusAction()
	{
		...
		$Id = $expensesModel->saveOrUpdateExpensesData($data, $where);
				...
				$finance = $expensesModel->getFinanceManager();
				$finance_manager_name = $finance[0]['userfullname'];
				$finance_manager_mail = $finance[0]['emailaddress'];

// 				print_r($finance);
// die();

				$this->view->finance_manager_name = $finance_manager_name;
				$this->view->finance_manager_mail = $finance_manager_mail;

				if($status=='approved')
				{
					$text2 = $view->render('expensemailtemplates/expensestatus2.phtml');
					$finance['subject'] = APPLICATION_NAME.': Approved Expenses';
					$finance['header'] = 'Greetings from Sentrifugo';
					$finance['toEmail'] = $finance_manager_mail;
					//$options['cc'] = 'sentritestone@gmail.com';

					$finance['toName'] = $finance_manager_name;
					$finance['message'] = $text2;
					$result = sapp_Global::_sendEmail($finance);
				}
	...
	{	
		$expense_model = new Expenses_Model_Expenses();
		$project_names = $this->_getParam('project_name');
		$expenses_data = $expense_model->getClientData($project_names);
		$client_name1 = $expenses_data['client'];
		$project_name1 = $expenses_data['project_name'];
		$this->_helper->json(array('client' =>$client_name1,'project' => $project_name1));	
		return $client;
	}
}
