...
class Expenses_Form_Expenses extends Zend_Form
{
	public function init()
	{
        ...
        $client_name = new Zend_Form_Element_Text('expense_client');
        $client_name->setLabel("Client");
        $client_name->setAttrib('maxLength', 50);
        $client_name->setAttrib('readonly','true');
        $client_name->addFilter(new Zend_Filter_StringTrim());
        $client_name->setRequired(false);
		...
		$this->addElements(array($id,$expense_name,$expense_date,$expense_amount,$base_project,$currency,$expense_category,$is_reimbursable,$expense_payment_methods,$expense_trips,$payment_ref_no,$is_from_advance,$description,$client_name,$submit));
