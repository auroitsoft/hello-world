...
class Timemanagement_Form_Projects extends Zend_Form
{
  ...
  //       $sow_number = $this->_request->getParam('sow_number');
		// $sow_amount = $this->_request->getParam('sow_amount');
		// $po_number = $this->_request->getParam('po_number');
		// $po_amount = $this->_request->getParam('po_amount');
		// $po_balance = $this->_request->getParam('po_balance');

		$sow_number = new Zend_Form_Element_Text('sow_number');
        $sow_number->setAttrib('maxLength', 100);
        // $sow_number->setRequired(true);

        $sow_amount = new Zend_Form_Element_Text('sow_amount');
        $sow_amount->setAttrib('maxLength', 100);
        // $sow_amount->setRequired(true);

        $po_number = new Zend_Form_Element_Text('po_number');
        $po_number->setAttrib('maxLength', 100);
        // $po_number->setRequired(true);

        $po_amount = new Zend_Form_Element_Text('po_amount');
        $po_amount->setAttrib('maxLength', 100);
        // $po_amount->setRequired(true);

        $po_balance = new Zend_Form_Element_Text('po_balance');
        $po_balance->setAttrib('maxLength', 100);
        // $po_balance->setRequired(true);
		...
		$submit->setLabel('Save');
		 ...
		 $this->addElements(array($id,$invoice_method,$project_name,$projstatus,$base_project,$description,$client,$currency,$project_type,$start_date,$end_date,$estimated_hrs,$submit,$sow_number,$sow_amount,$po_balance,$po_number,$po_amount));
